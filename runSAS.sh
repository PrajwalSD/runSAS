...
function create_a_sas_program_file_to_update_batch_status(){
    ...
    if [[ "$UPDATE_BATCH_STATUS_TO_SAS_DATASET_FLAG" == "Y" ]]; then
        ...
        # Debug entry
        print2debug r2s_sas_program_file_name "A new SAS program file has been (re)created and deployed to [$r2s_deploy_to_directory]"
    fi
}
#------
...
function runSAS(){
    ...
    function trigger_the_job_now(){
        if [[ $runsas_jobrc -eq $RC_JOB_PENDING ]]; then
            ...
            if [[ $running_jobs_current_count -lt $sjs_concurrent_job_count_limit ]]; then
                ...
                update_batch_status_to_sas_dataset "$global_batchid||$runsas_flowid||$runsas_flow||Triggered||$start_datetime_of_flow_timestamp_for_updsas||.||$runsas_jobid||$runsas_job||Triggered||.||.||$start_datetime_of_job_timestamp_for_updsas||.||PID:$runsas_job_pid"
    ...
    if [[ $runsas_jobrc -gt $runsas_max_jobrc ]]; then
        ...
        update_batch_status_to_sas_dataset "$global_batchid||$runsas_flowid||$runsas_flow||Failed||$start_datetime_of_flow_timestamp_for_updsas||.||$runsas_jobid||$runsas_job||Failed||${job_err_message//,/}||$runsas_job_log||$start_datetime_of_job_timestamp_for_updsas||$end_datetime_of_job_timestamp_for_updsas||PID:$runsas_job_pid" 
        ...
        # Update batch status to SAS dataset
        update_batch_status_to_sas_dataset "$global_batchid||$runsas_flowid||$runsas_flow||Complete||$start_datetime_of_flow_timestamp_for_updsas||.||$runsas_jobid||$runsas_job||Complete||.||$runsas_job_log||$start_datetime_of_job_timestamp_for_updsas||${end_datetime_of_job_timestamp_for_updsas:-"."}||PID:$runsas_job_pid"
...
for flow_file_name in `ls $RUNSAS_SPLIT_FLOWS_DIRECTORY/*.* | sort -V`; do
    ...
    update_batch_status_to_sas_dataset "$global_batchid||$runsas_flowid||$runsas_flow||Complete||$start_datetime_of_flow_timestamp_for_updsas||$end_datetime_of_flow_timestamp_for_updsas||.||.||.||.||.||.||.||PID:$runsas_job_pid"
