...
function check_if_batch_has_stalled(){
    ...
    # Stall check
    print2debug jobid "Stall check after full iter=&check_iter. for job [" "] batch_is_stalled=$batch_is_stalled | rc=$runsas_jobrc | runflag=$runflag | runsas_mode_runflag=$runsas_mode_runflag | runflag=$runflag | error_message_shown_on_job_fail=$error_message_shown_on_job_fail | cyclic_dependency_detected=$cyclic_dependency_detected | count_of_dep_jobs_currently_running=$count_of_dep_jobs_currently_running | count_of_jobs_currently_running=$count_of_jobs_currently_running"
    ...
    if [[ $batch_is_stalled -ge 1 ]]; then
        ...
        # Print to debug file
        print2debug stalled_msg_input "*** Batch has recovered from stall, user input was [" "] ***" 
    fi
}
#------
...
function refactor_job_list_file(){
    ...
    # Run mode 
    print2debug RUNSAS_RUNNING_IN_NO_FLOW_MODE "No flow mode flag is: [" "] "
...
#------
# Name: create_a_sas_program_file_to_update_batch_status()
# Desc: This function creates a simple SAS program to transfer the batch run status/info to a SAS dataset
#   In: <NA>
#  Out: <NA>
#------
function create_a_sas_program_file_to_update_batch_status(){
    # Parameters
    r2s_sas_dataset_name=${UPDATE_BATCH_STATUS_SAS_DATASET_LIBREF:-"runsas"}.$UPDATE_BATCH_STATUS_SAS_DATASET_NAME
    r2s_deploy_to_directory=${UPDATE_BATCH_STATUS_SAS_PROGRAM_FILE_DEPLOYED_DIRECTORY}
    r2s_sas_program_file_name=$UPDATE_BATCH_STATUS_SAS_PROGRAM_FILE_NAME

    # Check if this is required by the user
    if [[ "$UPDATE_BATCH_STATUS_TO_SAS_DATASET_FLAG" == "Y" ]]; then
        # Proceed with other checks...
        if [[ "$r2s_sas_dataset_name" == "" || "$r2s_deploy_to_directory" = "" || "$r2s_sas_program_file_name" == "" ]]; then
            printf "${red}\n\n*** ERROR: Missing a required parameter in SAS dataset update code routine ***${white}\n"
            clear_session_and_exit
        fi

        # Update the message bar
        publish_to_messagebar "${green}Generating SAS program file...${white}"

        # Create a SAS program file 
        echo "/* Updates the batch status info to a specified SAS dataset for tracking and reporting purposes */"                                                            > $r2s_sas_program_file_name
        echo "/* This file was auto-generated by runSAS on date '+%d-%m-%Y-%H:%M:%S' */"                                                                                    >> $r2s_sas_program_file_name
        echo ""                                                                                                                                                             >> $r2s_sas_program_file_name
        echo "%macro extract_sysparm_parameters;"                                                                                                                           >> $r2s_sas_program_file_name
        echo "      %put NOTE: The parameters passed in were: [%bquote(&=sysparm.)];"                                                                                       >> $r2s_sas_program_file_name
        echo "      %if \"&sysparm.\" ne \"\" %then %do;"                                                                                                                   >> $r2s_sas_program_file_name
        echo "          /* Get all positional parameters from sysparm */"                                                                                                   >> $r2s_sas_program_file_name
        echo "          %put NOTE: SYSPARM is non-empty, destructuring the SYSPARM...;"                                                                                     >> $r2s_sas_program_file_name
        echo "          %global batch_id "                                                                                                                                  >> $r2s_sas_program_file_name
        echo "                  flow_id "                                                                                                                                   >> $r2s_sas_program_file_name
        echo "                  flow_name "                                                                                                                                 >> $r2s_sas_program_file_name
        echo "                  flow_status "                                                                                                                               >> $r2s_sas_program_file_name
        echo "                  flow_start_time "                                                                                                                           >> $r2s_sas_program_file_name
        echo "                  flow_end_time "                                                                                                                             >> $r2s_sas_program_file_name
        echo "                  job_id "                                                                                                                                    >> $r2s_sas_program_file_name
        echo "                  job_name "                                                                                                                                  >> $r2s_sas_program_file_name
        echo "                  job_status "                                                                                                                                >> $r2s_sas_program_file_name
        echo "                  job_error_message "                                                                                                                         >> $r2s_sas_program_file_name
        echo "                  job_run_log "                                                                                                                               >> $r2s_sas_program_file_name
        echo "                  job_start_time "                                                                                                                            >> $r2s_sas_program_file_name
        echo "                  job_end_time"                                                                                                                               >> $r2s_sas_program_file_name
        echo "                  runsas_additional_info;"                                                                                                                    >> $r2s_sas_program_file_name
        echo "           %let batch_id=%scan(&sysparm.,1,'||');"                                                                                                            >> $r2s_sas_program_file_name
        echo "           %let flow_id=%scan(&sysparm.,2,'||');"                                                                                                             >> $r2s_sas_program_file_name
        echo "           %let flow_name=%scan(&sysparm.,3,'||');"                                                                                                           >> $r2s_sas_program_file_name
        echo "           %let flow_status=%scan(&sysparm.,4,'||');"                                                                                                         >> $r2s_sas_program_file_name
        echo "           %let flow_start_time=%scan(&sysparm.,5,'||');"                                                                                                     >> $r2s_sas_program_file_name
        echo "           %let flow_end_time=%scan(&sysparm.,6,'||');"                                                                                                       >> $r2s_sas_program_file_name
        echo "           %let job_id=%scan(&sysparm.,7,'||');"                                                                                                              >> $r2s_sas_program_file_name
        echo "           %let job_name=%scan(&sysparm.,8,'||');"                                                                                                            >> $r2s_sas_program_file_name
        echo "           %let job_status=%scan(&sysparm.,9,'||');"                                                                                                          >> $r2s_sas_program_file_name
        echo "           %let job_error_message=%scan(&sysparm.,10,'||');"                                                                                                  >> $r2s_sas_program_file_name
        echo "           %let job_run_log=%scan(&sysparm.,11,'||');"                                                                                                        >> $r2s_sas_program_file_name
        echo "           %let job_start_time=%scan(&sysparm.,12,'||');"                                                                                                     >> $r2s_sas_program_file_name
        echo "           %let job_end_time=%scan(&sysparm.,13,'||');"                                                                                                       >> $r2s_sas_program_file_name
        echo "           %let runsas_additional_info=%scan(&sysparm.,14,'||');"                                                                                             >> $r2s_sas_program_file_name
        echo "      %end;"                                                                                                                                                  >> $r2s_sas_program_file_name
        echo "      %else %do;"                                                                                                                                             >> $r2s_sas_program_file_name
        echo "          %put ERROR: No parameters were passed to this program [%sysfunc(compbl(&=sysparm.))]...;"                                                           >> $r2s_sas_program_file_name
        echo "      %end;"                                                                                                                                                  >> $r2s_sas_program_file_name
        echo "%mend;"                                                                                                                                                       >> $r2s_sas_program_file_name
        echo ""                                                                                                                                                             >> $r2s_sas_program_file_name
        echo "%macro update_batch_status;"                                                                                                                                  >> $r2s_sas_program_file_name
        echo "  %if \"&sysparm.\" ne \"\" %then %do;"                                                                                                                       >> $r2s_sas_program_file_name
        echo "      %if %sysfunc(exist($r2s_sas_dataset_name)) %then %do;"                                                                                                  >> $r2s_sas_program_file_name
        echo "              %put NOTE: The runSAS batch status dataset [$r2s_sas_dataset_name] exists;"                                                                     >> $r2s_sas_program_file_name
        echo "      %end;"                                                                                                                                                  >> $r2s_sas_program_file_name
        echo "      %else %do;"                                                                                                                                             >> $r2s_sas_program_file_name
        echo "              %put NOTE: The runSAS batch status dataset does not exist, creating one [$r2s_sas_dataset_name]...;"                                            >> $r2s_sas_program_file_name
        echo "              data $r2s_sas_dataset_name;"                                                                                                                    >> $r2s_sas_program_file_name
        echo "                  format batch_id 8.;"                                                                                                                        >> $r2s_sas_program_file_name
        echo "                  format flow_id 8.;"                                                                                                                         >> $r2s_sas_program_file_name
        echo "                  format flow_name flow_status \$250.;"                                                                                                       >> $r2s_sas_program_file_name
        echo "                  format flow_start_time flow_end_time datetime20.;"                                                                                          >> $r2s_sas_program_file_name
        echo "                  format job_id 8.;"                                                                                                                          >> $r2s_sas_program_file_name
        echo "                  format job_name job_status \$250.;"                                                                                                         >> $r2s_sas_program_file_name
        echo "                  format job_error_message job_run_log \$1000.;"                                                                                              >> $r2s_sas_program_file_name
        echo "                  format job_start_time job_end_time datetime20.;"                                                                                            >> $r2s_sas_program_file_name
        echo "                  format runsas_additional_info \$1000.;"                                                                                                     >> $r2s_sas_program_file_name
        echo "                  stop;"                                                                                                                                      >> $r2s_sas_program_file_name
        echo "              run;"                                                                                                                                           >> $r2s_sas_program_file_name
        echo "      %end;"                                                                                                                                                  >> $r2s_sas_program_file_name
        echo ""                                                                                                                                                             >> $r2s_sas_program_file_name
        echo "      %let latest_batch_id=0; "                                                                                                                               >> $r2s_sas_program_file_name
        echo "      %let batch_id_exists=0; "                                                                                                                               >> $r2s_sas_program_file_name
        echo "      %let flow_id_exists=0; "                                                                                                                                >> $r2s_sas_program_file_name
        echo "      %let job_id_exists=0; "                                                                                                                                 >> $r2s_sas_program_file_name
        echo "      proc sql noprint;"                                                                                                                                      >> $r2s_sas_program_file_name
        echo "          select max(batch_id) into :latest_batch_id from $r2s_sas_dataset_name;"                                                                             >> $r2s_sas_program_file_name
        echo "          select 1 into :batch_id_exists from $r2s_sas_dataset_name where batch_id=&batch_id.;"                                                               >> $r2s_sas_program_file_name 
        echo "          %if &batch_id_exists. %then %do;"                                                                                                                   >> $r2s_sas_program_file_name                    
        echo "              %if &flow_id. ne \"\" and &flow_id. ne \".\" %then %do;"                                                                                        >> $r2s_sas_program_file_name
        echo "                  select 1 into :flow_id_exists from $r2s_sas_dataset_name"                                                                                   >> $r2s_sas_program_file_name 
        echo "                      where batch_id=&batch_id. and flow_id=&flow_id.;"                                                                                       >> $r2s_sas_program_file_name
        echo "              %end;"                                                                                                                                          >> $r2s_sas_program_file_name
        echo "              %if &job_id. ne \"\" and &job_id. ne \".\" %then %do;"                                                                                          >> $r2s_sas_program_file_name
        echo "                  select 1 into :job_id_exists from $r2s_sas_dataset_name"                                                                                    >> $r2s_sas_program_file_name 
        echo "                      where batch_id=&batch_id. and flow_id=&flow_id. and job_id=&job_id.;"                                                                   >> $r2s_sas_program_file_name
        echo "              %end;"                                                                                                                                          >> $r2s_sas_program_file_name
        echo "          %end;"                                                                                                                                              >> $r2s_sas_program_file_name
        echo "      quit;"                                                                                                                                                  >> $r2s_sas_program_file_name
        echo "      %put NOTE: [&=batch_id.,&=batch_id_exists.|&=flow_id.,&=flow_id_exists.|&=job_id.,&=job_id_exists.];"                                                   >> $r2s_sas_program_file_name
        echo ""                                                                                                                                                             >> $r2s_sas_program_file_name
        echo "      %if &batch_id_exists. and &flow_id_exists. and &job_id_exists. %then %do;"                                                                              >> $r2s_sas_program_file_name
        echo "          /* Update (job-level) */"                                                                                                                           >> $r2s_sas_program_file_name
        echo "          %put NOTE: Batch, flow and job exists in the table, updating [&=batch_id.|&=flow_id.|&=job_id.]...;"                                                >> $r2s_sas_program_file_name
        echo "          proc sql noprint;"                                                                                                                                  >> $r2s_sas_program_file_name
        echo "              update $r2s_sas_dataset_name"                                                                                                                   >> $r2s_sas_program_file_name
        echo "              set flow_start_time=\"&flow_start_time.\"dt, "                                                                                                  >> $r2s_sas_program_file_name
        echo "                  flow_end_time=\"&flow_end_time.\"dt,"                                                                                                       >> $r2s_sas_program_file_name
        echo "                  flow_status=\"&flow_status.\""                                                                                                              >> $r2s_sas_program_file_name
        echo "                  %if \"&job_id.\" ne \"\" and \"&job_id.\" ne \".\" %then %do;"                                                                              >> $r2s_sas_program_file_name
        echo "                      ,job_start_time=\"&job_start_time.\"dt,"                                                                                                >> $r2s_sas_program_file_name
        echo "                      job_end_time=\"&job_end_time.\"dt,"                                                                                                     >> $r2s_sas_program_file_name
        echo "                      job_status=\"&job_status.\","                                                                                                           >> $r2s_sas_program_file_name
        echo "                      job_error_message=\"&job_error_message.\","                                                                                             >> $r2s_sas_program_file_name
        echo "                      job_run_log=\"&job_run_log.\","                                                                                                         >> $r2s_sas_program_file_name
        echo "                      runsas_additional_info=\"&runsas_additional_info.\""                                                                                    >> $r2s_sas_program_file_name
        echo "                  %end;"                                                                                                                                      >> $r2s_sas_program_file_name
        echo "              where batch_id=&batch_id. and flow_id=&flow_id. and job_id=&job_id."                                                                            >> $r2s_sas_program_file_name
        echo "              ;"                                                                                                                                              >> $r2s_sas_program_file_name
        echo "        quit;"                                                                                                                                                >> $r2s_sas_program_file_name
        echo "      %end;"                                                                                                                                                  >> $r2s_sas_program_file_name
        echo "      %else %if &batch_id_exists. and &flow_id_exists. %then %do;"                                                                                            >> $r2s_sas_program_file_name
        echo "          /* Update (flow-level) */"                                                                                                                          >> $r2s_sas_program_file_name
        echo "          %put NOTE: Batch and flow exists, updating [&=batch_id.|&=flow_id.]...;"                                                                            >> $r2s_sas_program_file_name
        echo "          proc sql noprint;"                                                                                                                                  >> $r2s_sas_program_file_name
        echo "              update $r2s_sas_dataset_name"                                                                                                                   >> $r2s_sas_program_file_name
        echo "              set flow_start_time=\"&flow_start_time.\"dt, "                                                                                                  >> $r2s_sas_program_file_name
        echo "                  flow_end_time=\"&flow_end_time.\"dt,"                                                                                                       >> $r2s_sas_program_file_name
        ...
        echo "      %end;"                                                                                                                                                  >> $r2s_sas_program_file_name
        echo "      %else %do;"                                                                                                                                             >> $r2s_sas_program_file_name
        echo "          /* New record (new batch or a new flow or a new job run) */"                                                                                        >> $r2s_sas_program_file_name
        echo "          %put NOTE: Inserting a new record for [&=batch_id.|&=flow_id.|&=job_id.] as [&=batch_id_exists.|&=flow_id_exists.|&=job_id_exists.]...;"            >> $r2s_sas_program_file_name
        echo "          proc sql noprint;"                                                                                                                                  >> $r2s_sas_program_file_name
        echo "              insert into $r2s_sas_dataset_name(batch_id, "                                                                                                   >> $r2s_sas_program_file_name
        echo "                                                 flow_id, "                                                                                                   >> $r2s_sas_program_file_name
        echo "                                                 flow_name, "                                                                                                 >> $r2s_sas_program_file_name
        echo "                                                 flow_status, "                                                                                               >> $r2s_sas_program_file_name
        echo "                                                 flow_start_time, "                                                                                           >> $r2s_sas_program_file_name
        ...
        echo "                                                 job_run_log, "                                                                                               >> $r2s_sas_program_file_name
        echo "                                                 runsas_additional_info)"                                                                                     >> $r2s_sas_program_file_name
        echo "              values(&batch_id., "                                                                                                                            >> $r2s_sas_program_file_name
        echo "                     &flow_id., "                                                                                                                             >> $r2s_sas_program_file_name
        echo "                     \"&flow_name.\", "                                                                                                                       >> $r2s_sas_program_file_name
        echo "                     \"&flow_status.\", "                                                                                                                     >> $r2s_sas_program_file_name
        echo "                     \"&flow_start_time.\"dt, "                                                                                                               >> $r2s_sas_program_file_name
        echo "                     \"&flow_end_time.\"dt, "                                                                                                                 >> $r2s_sas_program_file_name
        echo "                     &job_id., "                                                                                                                              >> $r2s_sas_program_file_name
        echo "                     \"&job_name.\", "                                                                                                                        >> $r2s_sas_program_file_name
        echo "                     \"&job_status.\", "                                                                                                                      >> $r2s_sas_program_file_name
        echo "                     \"&job_start_time.\"dt, "                                                                                                                >> $r2s_sas_program_file_name
        echo "                     \"&job_end_time.\"dt, "                                                                                                                  >> $r2s_sas_program_file_name
        echo "                     \"&job_error_message.\", "                                                                                                               >> $r2s_sas_program_file_name
        echo "                     \"&job_run_log.\", "                                                                                                                     >> $r2s_sas_program_file_name
        echo "                     \"&runsas_additional_info.\");"                                                                                                          >> $r2s_sas_program_file_name
        echo "          quit;"                                                                                                                                              >> $r2s_sas_program_file_name
        ...
        echo "%mend;"                                                                                                                                                       >> $r2s_sas_program_file_name
        echo ""                                                                                                                                                             >> $r2s_sas_program_file_name
        if [[ "$UPDATE_BATCH_STATUS_SAS_DATASET_LIBREF" == "runsas" ]]; then
            echo "libname runsas \"${UPDATE_BATCH_STATUS_SAS_DATASET_PATH}\";"                                                                                              >> $r2s_sas_program_file_name
        fi
        echo ""                                                                                                                                                             >> $r2s_sas_program_file_name
        echo "/* Execute */"                                                                                                                                                >> $r2s_sas_program_file_name
        echo "%extract_sysparm_parameters;"                                                                                                                                 >> $r2s_sas_program_file_name
        echo ""                                                                                                                                                             >> $r2s_sas_program_file_name
        echo "/* Print all parameters for debugging */"                                                                                                                     >> $r2s_sas_program_file_name
        echo "%put NOTE: &=batch_id;"                                                                                                                                       >> $r2s_sas_program_file_name
        echo "%put NOTE: &=flow_id;"                                                                                                                                        >> $r2s_sas_program_file_name
        echo "%put NOTE: &=flow_name;"                                                                                                                                      >> $r2s_sas_program_file_name
        echo "%put NOTE: &=flow_status;"                                                                                                                                    >> $r2s_sas_program_file_name
        echo "%put NOTE: &=flow_start_time;"                                                                                                                                >> $r2s_sas_program_file_name
        echo "%put NOTE: &=flow_end_time;"                                                                                                                                  >> $r2s_sas_program_file_name
        echo "%put NOTE: &=job_id;"                                                                                                                                         >> $r2s_sas_program_file_name
        echo "%put NOTE: &=job_name;"                                                                                                                                       >> $r2s_sas_program_file_name
        echo "%put NOTE: &=job_status;"                                                                                                                                     >> $r2s_sas_program_file_name
        echo "%put NOTE: &=job_error_message;"                                                                                                                              >> $r2s_sas_program_file_name
        echo "%put NOTE: &=job_run_log;"                                                                                                                                    >> $r2s_sas_program_file_name
        echo "%put NOTE: &=job_start_time;"                                                                                                                                 >> $r2s_sas_program_file_name
        echo "%put NOTE: &=job_end_time;"                                                                                                                                   >> $r2s_sas_program_file_name
        echo "%put NOTE: &=runsas_additional_info;"                                                                                                                         >> $r2s_sas_program_file_name
        echo ""                                                                                                                                                             >> $r2s_sas_program_file_name
        echo "/* Debug */"                                                                                                                                                  >> $r2s_sas_program_file_name
        echo "options mprint symbolgen merror;"                                                                                                                             >> $r2s_sas_program_file_name
        echo ""                                                                                                                                                             >> $r2s_sas_program_file_name
        echo "/* Execute */"                                                                                                                                                >> $r2s_sas_program_file_name
        echo "%update_batch_status;"                                                                                                                                        >> $r2s_sas_program_file_name

        # Update the message bar
        publish_to_messagebar "${green}Deploying SAS program file...${white}"

        # Deploy the SAS file to a specified directory 
        mv $r2s_sas_program_file_name $r2s_deploy_to_directory

        # Check if the file was deployed and change the perms
        check_if_the_file_exists $r2s_deploy_to_directory/$r2s_sas_program_file_name
        chmod 775 $r2s_deploy_to_directory/$r2s_sas_program_file_name

        # Debug entry
        print2debug r2s_sas_program_file_name "NOTE: Batch status update SAS program file has been (re-)created [" "] and deployed to [$r2s_deploy_to_directory]"
...
#------
# Name: update_batch_status_to_sas_dataset()
# Desc: This function updates the batch status to the SAS dataset
#   In: batch_id,flow_id,flow_name,flow_status,flow_start_time,flow_end_time,job_id,job_name,job_status,job_error_message,job_run_log,job_start_time,job_end_time,runsas_additional_info
#  Out: <NA>
#------
function update_batch_status_to_sas_dataset(){
    # Input parameters
    upd_batchstatus_batch_id=$1
    upd_batchstatus_flow_id=$2
    upd_batchstatus_flow_name=$3
    upd_batchstatus_flow_status=$4
    upd_batchstatus_flow_start_time=$5
    upd_batchstatus_flow_end_time=$6
    upd_batchstatus_job_id=$7
    upd_batchstatus_job_name=$8
    upd_batchstatus_job_status=$9
    upd_batchstatus_job_error_message=${10}
    upd_batchstatus_job_run_log=${11}
    upd_batchstatus_job_start_time=${12}
    upd_batchstatus_job_end_time=${13}
    upd_batchstatus_runsas_additional_info=${14}

    # Other parameters
    runsas_update_batch_status_err_file=$RUNSAS_BATCH_STATE_ROOT_DIRECTORY/$global_batchid/.runsas_update_batch_status.err

    # Message bar update
    publish_to_messagebar "${green}runSAS is updating batch status dataset in SAS...${white}"
    ...
    if [[ "$UPDATE_BATCH_STATUS_TO_SAS_DATASET_FLAG" == "Y" ]]; then
        nice -n 20 $runsas_batch_server_root_directory/$runsas_sh   -log $runsas_logs_root_directory/${UPDATE_BATCH_STATUS_SAS_PROGRAM_FILE_NAME}_#Y.#m.#d_#H.#M.#s.log \
                                                                    ...
                                                                    -sysin $runsas_deployed_jobs_root_directory/$UPDATE_BATCH_STATUS_SAS_PROGRAM_FILE_NAME \
                                                                    -sysparm "$upd_batchstatus_batch_id \
                                                                              $upd_batchstatus_flow_id \
                                                                              $upd_batchstatus_flow_name \
                                                                              $upd_batchstatus_flow_status \
                                                                              $upd_batchstatus_flow_start_time \
                                                                              $upd_batchstatus_flow_end_time \
                                                                              $upd_batchstatus_job_id \
                                                                              $upd_batchstatus_job_name \
                                                                              $upd_batchstatus_job_status \
                                                                              $upd_batchstatus_job_error_message \
                                                                              $upd_batchstatus_job_run_log \
                                                                              $upd_batchstatus_job_start_time \
                                                                              $upd_batchstatus_job_end_time \
                                                                              $upd_batchstatus_runsas_additional_info \
                                                                             "
    fi

    # Get the log file name
    runsas_update_batch_status_job_log=`ls -tr $runsas_logs_root_directory/${UPDATE_BATCH_STATUS_SAS_PROGRAM_FILE_NAME}*.log 2> /dev/null | tail -1`
    runsas_update_batch_status_job_log=${runsas_update_batch_status_job_log##/*/}

    # Check if there are any errors in the log
    if [[ -f "$runsas_logs_root_directory/$runsas_update_batch_status_job_log" ]]; then
        $RUNSAS_LOG_SEARCH_FUNCTION -m${JOB_ERROR_DISPLAY_COUNT} \
                                    ...
                                    $runsas_logs_root_directory/$runsas_update_batch_status_job_log > $runsas_update_batch_status_err_file
    else
        touch $runsas_update_batch_status_err_file
    fi

    # Again, suppress unwanted lines in the log (typical SAS errors!)
    remove_a_line_from_file ^$ "$runsas_update_batch_status_err_file"
    remove_a_line_from_file "$SUPPRESS_ERROR_MESSAGE_1" "$runsas_update_batch_status_err_file"
    remove_a_line_from_file "$SUPPRESS_ERROR_MESSAGE_2" "$runsas_update_batch_status_err_file"
    remove_a_line_from_file "$SUPPRESS_ERROR_MESSAGE_3" "$runsas_update_batch_status_err_file"
    remove_a_line_from_file "$SUPPRESS_ERROR_MESSAGE_4" "$runsas_update_batch_status_err_file"
    remove_a_line_from_file "$SUPPRESS_ERROR_MESSAGE_5" "$runsas_update_batch_status_err_file"
    remove_a_line_from_file "$SUPPRESS_ERROR_MESSAGE_6" "$runsas_update_batch_status_err_file"
    remove_a_line_from_file "$SUPPRESS_ERROR_MESSAGE_7" "$runsas_update_batch_status_err_file"
    remove_a_line_from_file "$SUPPRESS_ERROR_MESSAGE_8" "$runsas_update_batch_status_err_file"
    remove_a_line_from_file "$SUPPRESS_ERROR_MESSAGE_9" "$runsas_update_batch_status_err_file"
    remove_a_line_from_file "$SUPPRESS_ERROR_MESSAGE_10" "$runsas_update_batch_status_err_file"

    # See if the error file has been generated (generated when there's an error)
    if [ -s $runsas_update_batch_status_err_file ]; then
        printf "${red}\n*** ERROR: Update batch status to SAS dataset routine has errored, more details in the log ($runsas_logs_root_directory/$runsas_update_batch_status_job_log) ***\n${white}"
        clear_session_and_exit
    fi
...
function runSAS(){
    ...
    # Print to debug file
    print2debug runsas_job "===> runSAS() has been called with [" "] for runsas_flowid=[$runsas_flowid] and runsas_jobid=[$runsas_jobid] <==="
    ...
    function trigger_the_job_now(){
        if [[ $runsas_jobrc -eq $RC_JOB_PENDING ]]; then 
            if [[ $runsas_job_pid -eq 0 ]]; then # This attempts to avoid re-triggering the job with same jobid (i.e., no pid assigned for the jobid)
                ...
                print2debug runsas_job "*** WARNING: Detected a re-triggering event for [" "] job [runsas_jobid=${runsas_jobid} | runsas_job_pid=${runsas_job_pid} | runsas_jobrc=${runsas_jobrc}], but runSAS skipped this)***"  
            fi
        fi
    ...
    if [[ "$runsas_jobdep" == "" ]] || [[ "$runsas_jobdep" == "$runsas_jobid" ]]; then
            ...
            print2debug runsas_jobdep "*** Trigger request - no/self dependency [" "] ***"
            trigger_the_job_now
    else
        ...
        do                 
            ...
            if [[ $runsas_logic_op == "OR" ]]; then
                if [[ $OR_check_passed -eq 1 ]]; then
                    print2debug runsas_jobid "*** Triggering request - OR check " " ***" 
                    # Trigger!
                    trigger_the_job_now   
                fi
            else   
                 if [[ $AND_check_passed -eq 1 ]]; then 
                    print2debug runsas_jobid "*** Triggering request - AND check " " ***" 
                    # Trigger!
                    trigger_the_job_now
                fi             
            fi
    ...
    if [[ $runsas_jobrc -gt $runsas_max_jobrc ]]; then
        ...
        end_datetime_of_job_timestamp_for_updsas="`date '+%d%b%Y:%H:%M:%S'`"
...
UPDATE_BATCH_STATUS_SAS_PROGRAM_FILE_NAME=runsas_update_batch_status.sas                            # Specify the name of the SAS file generated by runSAS to implement the update batch status routine
UPDATE_BATCH_STATUS_SAS_PROGRAM_FILE_DEPLOYED_DIRECTORY=$SAS_DEPLOYED_JOBS_ROOT_DIRECTORY           # This is where the SAS file will be deployed to, DO NOT CHANGE THIS 
...
if [[ $RUNSAS_INVOKED_IN_RESUME_MODE -le -1 ]] && [[ $RUNSAS_INVOKED_IN_UPDATE_MODE -le -1 ]] && [[ $RUNSAS_INVOKED_IN_LIST_MODE -le -1 ]] && [[ $RUNSAS_INVOKED_IN_JOBLIST_MODE -le -1 ]] && [[ $RUNSAS_INVOKED_IN_REDEPLOY_MODE -le -1 ]] && [[ $RUNSAS_INVOKED_IN_LOG_MODE -le -1 ]]; then
...
# Check if user has requested for SAS dataset batch status update feature
create_a_sas_program_file_to_update_batch_status
...
for flow_file_name in `ls $RUNSAS_SPLIT_FLOWS_DIRECTORY/*.* | sort -V`; do
    ...
    # Capture flow runtimes
    start_datetime_of_flow_timestamp=`date '+%d-%m-%Y-%H:%M:%S'`
    start_datetime_of_flow_timestamp_for_updsas=`date '+%d%b%Y:%H:%M:%S'`
    start_datetime_of_flow=`date +%s`
    ...
    # Capture flow runtimes
    end_datetime_of_flow_timestamp=`date '+%d-%m-%Y-%H:%M:%S'`
    end_datetime_of_flow_timestamp_for_updsas=`date '+%d%b%Y:%H:%M:%S'`
    end_datetime_of_flow=`date +%s`
